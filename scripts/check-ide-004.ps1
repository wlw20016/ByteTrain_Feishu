...
$paths = @{
    ...
    Helper = Join-Path $root "scripts/ide-build.ps1"
    WorkflowDoc = Join-Path $root "docs/ai-context/ide-bazel-workflow.md"
}
...
foreach ($command in $scriptCommands) {
    ...
    if ($helperText -notmatch [regex]::Escape($command.Target)) {
        Add-Failure "scripts/ide-build.ps1 supports target $($command.Target)"
    }
...
if ($null -ne $tasksJson) {
    ...
    foreach ($command in $scriptCommands) {
        $matchingTask = $tasks | Where-Object {
            ...
            $hasHelper = ($args -join " ") -match [regex]::Escape("scripts/ide-build.ps1")
