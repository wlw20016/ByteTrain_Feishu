...
The IDE plugin prototype MUST invoke `scripts/ide-build.ps1` for build, test, and query commands instead of duplicating Bazel command strings.

#### Scenario: User runs app build from IDE plugin

- **WHEN** the user selects the app build command from the plugin prototype
- **THEN** the plugin invokes `scripts/ide-build.ps1 -Target app`
...
- **THEN** the documentation identifies whether to use VS Code tasks, plugin commands, or direct `scripts/ide-build.ps1` invocation
