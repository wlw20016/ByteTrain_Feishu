...
只有在 Bazel run 行为定义清楚后，`scripts/ide-build.ps1` 才应新增类似 `run-app` 的运行 target。VS Code plugin 和 `.vscode/tasks.json` 应继续调用该脚本，而不是复制命令。
...
可复用失败应记录到 `docs/ai-context/common-build-errors.md`。
