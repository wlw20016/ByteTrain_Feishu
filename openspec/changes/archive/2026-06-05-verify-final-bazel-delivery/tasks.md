...
- [x] BZL-FINAL-009 如果任何 Bazel 命令出现 `Access is denied` 等环境输出，在 `docs/ai-context/build-system/common-build-errors.md` 中记录根因和修复方式。
- [x] BZL-FINAL-010 环境修复后重新运行被阻塞命令，或保留明确的阻塞记录、负责人和重试命令。

## 4. 文档

- [x] BZL-FINAL-011 更新 `docs/ai-context/build-system/build-commands.md`，记录最终验证结果摘要。
- [x] BZL-FINAL-012 如果最终 query 输出与已记录依赖边界不同，更新 `docs/project/module-boundaries.md`。
...
入口脚本：`scripts/commands/verify-final-bazel-delivery.ps1`。脚本按 app、proto、shared/feature Kotlin、Rust SDK test、app dependency query 顺序运行 Bazel，失败即停，并写入 `docs/evidence/final-bazel-delivery-evidence.md`。脚本不调用 Gradle；iOS/Xcode 因 Android-only 交付跳过。
...
- 环境闭环：普通受限入口中 `bazel.cmd`、`bazel.exe` 和 `bazelisk.exe` 曾输出 `Access is denied`/`拒绝访问`。允许执行本机 Bazel 后，阻塞的 app build 已重跑通过，最终脚本完整通过。根因和重试命令已记录在 `docs/ai-context/build-system/common-build-errors.md`。
