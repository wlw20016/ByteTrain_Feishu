...
这是 ByteTrain IDE helper commands 的最小 VS Code extension prototype。它保持 `scripts/ide-build.ps1` 作为唯一脚本命令来源，插件只负责把 IDE commands 映射到 helper targets。
...
| `bytetrain.bazelHelper.openBuildCommands` | Bazel: Open Build Commands | 打开 `docs/ai-context/build-commands.md`。 |
| `bytetrain.bazelHelper.openModuleBoundaries` | Bazel: Open Module Boundaries | 打开 `docs/ai-context/module-boundaries.md`。 |
| `bytetrain.bazelHelper.openCommonBuildErrors` | Bazel: Open Common Build Errors | 打开 `docs/ai-context/common-build-errors.md`。 |
...
- App run 使用 `scripts/ide-build.ps1 -Target run-app`，该入口调用 Bazel run wrapper。
