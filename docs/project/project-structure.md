...
- `.vscode/`：VS Code/Trae tasks，调用 `scripts/commands/ide-build.ps1`。
...
主要 target 记录在 `docs/project/module-boundaries.md` 和 `docs/ai-context/build-system/build-commands.md`：
...
- `docs/evidence/final-bazel-delivery-evidence.md`：最终 Bazel app/proto/feature/Rust/query evidence。
- `docs/evidence/final-ai-evidence.md`：最终 AI prompt、建议、人工决策和结果汇总。
- `docs/evidence/openspec-archive-audit.md`：OpenSpec validate、superseded 和 archive 决策。
- `docs/evidence/final-retrospective.md`：可复用 prompt、有效/无效建议和剩余限制。
- `docs/ai-context/build-system/common-build-errors.md`：真实构建失败、环境阻塞、根因、修复和重试命令。
