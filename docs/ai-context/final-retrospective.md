...
- “请把构建失败按命令、输出摘要、根因、修复方式、重试命令记录到 `docs/ai-context/common-build-errors.md`。”
- “请用 Bazel query 验证 app、feature、shared 的显式依赖边界，并同步 `docs/ai-context/module-boundaries.md`。”
...
- Trae/VS Code 构建入口复用同一 `scripts/ide-build.ps1`。
- 最终 `scripts/verify-final-bazel-delivery.ps1` 生成 concise evidence。
